...
class ChordDrawer {
    constructor(element, options) {
        ...
        // console.log('Chord.Drawer()->data', this.data.chord.length)

        let fretboardRange = this.data.chordLength
        ...
        let svgWidthBuffer = this.riffWidth - (this.stringHorizontalMargin / 2)
    ...
    _drawClampString(fret, string, finger = 0) {
        ...
        let cxUnit = this.riffStartX + this.stringHorizontalMargin / 2 + this.stringHorizontalMargin;
        let cx = cxUnit * (fret - 1);
        let cy = this.riffStartY + this.stringVerticalMargin * (string - 1);

        // check for fretboardStart offset
        // if (this.data.fretboardStart >= 1) {
            // cx -= cxUnit * 0
        // }
    ...
    _drawBarre(fret, from, to) {
        ...
        let xUnit = this.riffStartX + this.stringHorizontalMargin / 2 - this.radiusClampString + this.stringHorizontalMargin;
        let x = xUnit * (fret - rootFret)
    ...
    _drawFretNumber(rootFret, options) {
        ...
        for (let i = 0; i <= 2; i++) {
            ...
            let preText = '';
            if (this.data.meta.fretboardText) {
                preText = this.data.meta.fretboardText.pre || ''
            }
            textElement.textContent = preText + rootFret;
    ...
    _drawTitle() {
        ...
        let display = !(this.data.meta.title && this.data.meta.title.display === 'hide')
        if (display) {
            this.svg.appendChild(textElement);
        }
    };
