export const GUTTER_SMALL = 3;
export const GUITAR_STRING_START = 23;
export const GUITAR_STRING_MARGIN = 20;
export const GUITAR_NECK_START = 18;
export const FRET_START = 85;
export const FRET_MARGIN = 60;
export const BARRE_START = 55;
export const BARRE_MARGIN = 60;
export const GUITAR_STRINGS = [0, 1, 2, 3, 4, 5];
export const GUITAR_STRINGS_STATUS = ['closed', 'closed', 'closed', 'closed', 'closed', 'closed'];
export const STRINGS_STATUS_MAP = {
  yes: 'muted',
  no: 'closed',
  open: 'open'
};